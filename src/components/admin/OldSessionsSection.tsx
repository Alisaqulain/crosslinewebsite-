...
export function OldSessionsSection() {
  ...
  return (
    <div>
      <p className="text-sm text-slate-600 mb-4">
        Past sessions before the booking system — add or edit here on the same page as current bookings.
      </p>

      <div className="grid sm:grid-cols-2 gap-3 mb-6 max-w-md">
        <Card className="!p-4">
          <p className="text-xs text-slate-500">Total received</p>
          <p className="text-lg font-bold text-green-700">{formatCurrency(totals.received)}</p>
        </Card>
        <Card className="!p-4">
          <p className="text-xs text-slate-500">Udhari pending</p>
          <p className="text-lg font-bold text-red-600">{formatCurrency(totals.udhari)}</p>
        </Card>
      </div>
      ...
      <Card className="p-0 md:p-6">
        <ResponsiveTable
          data={matches}
          rowKey={(m) => m.id}
          emptyMessage="No old sessions yet"
          columns={[
